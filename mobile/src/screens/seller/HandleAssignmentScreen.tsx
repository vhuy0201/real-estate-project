...
export const HandleAssignmentScreen: React.FC = () => {
  ...
  const { data: assignmentsData, isLoading, error } = useGetSellerAssignments(
    {
      status: statusFilter !== 'all' ? (statusFilter as (typeof statusFilter) & ('pending' | 'accepted' | 'rejected' | 'cancelled')) : undefined,
    },
    true,
  );

  const { mutateAsync: mutateAcceptAssignment, isPending: isAccepting } =
    useAcceptAssignment();
  const { mutateAsync: mutateRejectAssignment, isPending: isRejecting } =
    useRejectAssignment();
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      await queryClient.refetchQueries({ queryKey: ['sellerAssignments'] });
    } finally {
      setRefreshing(false);
    }
  }, [queryClient]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Yêu cầu Agent</Text>
        <Text style={styles.headerSubtitle}>
          Quản lý yêu cầu từ các agent
        </Text>
      </View>
