...
export const getMyDealById = async (req: Request, res: Response) => {
  try {
    ...
    const deal = await dealService.getDealForAgent(dealId, String(agentId));
    if (!deal) {
    ...
    console.error("getMyDealById (agent) error:", error);
...
export const getMyDeals = async (req: Request, res: Response) => {
  try {
    ...
    const deals = await dealService.getDealsForAgent(String(agentId), {
    ...
    console.error("getMyDeals (agent) error:", error);
    const status = error?.status || 500;
